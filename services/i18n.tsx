...
const dictionaries: Record<Language, Translations> = {
  en: {
    appTitle: 'Investment Tool',
    // Config Panel
    profiles: 'Profiles',
    addProfile: 'Add Profile',
    autoGenerate: 'Auto Generate Combinations',
    confirmAutoGenerate:
      'This will auto-generated combinations profiles based on seed profile. Continue?',
    copyPrefix: 'Copy- ',
    ...
    endMonth: 'End Month',
    availableDataWindow: 'Available Data',
    ...
    bankruptcyLtv:
      'The following strategies exceeded the maximum LTV limit and were liquidated:',
    bankruptcyNegativeCash: 'The following strategies went bankrupt due to negative cash balance:',
    bankruptcyWithdrawal:
      'The following strategies went bankrupt because withdrawal exceeded available cash and sellable assets:',
    bankruptcyExpense:
      'The following strategies could not cover their annual living expenses:',
    bankruptcyDesc:
      'The following strategies failed adequacy checks and were liquidated:',
    ...
    year: 'Year',
    cf_deposit: 'Deposit',
    cf_withdrawal: 'Withdrawal',
    cf_borrowing: 'Borrowing',
    cf_net: 'Net',
    perfComparison: 'Performance Comparison',
    ltvChartTitle: 'LTV / Risk Tracking',
    ltvChartDesc:
      'Tracks the Loan-to-Value (LTV) ratio over time. Higher LTV increases liquidation risk.',
    betaChartTitle: 'Portfolio Beta (Risk Exposure)',
    betaChartDesc:
      'Weighted average Beta (QQQ=1, QLD=2, Cash=0) relative to Net Equity. Beta > 1 implies leverage.',
    ...
    math_cagr: 'CAGR',
    math_cagr_expl:
      'Compound Annual Growth Rate. Measures the mean annual growth rate of an investment assuming profits are reinvested.',
    math_cagr_formula: 'CAGR = (End Value / Start Value)^(1/n) - 1',
    math_irr: 'IRR',
    math_irr_expl:
      'Internal Rate of Return. The discount rate that makes the net present value (NPV) of all cash flows equal to zero. Solved via Newton-Raphson method.',
    ...
    math_calmar_expl:
      'Measures return relative to drawdown risk. Using IRR as the numerator accounts for the timing of cash flows, making it more accurate for DCA strategies.',
    math_calmar_formula: 'Calmar = IRR / Max Drawdown',
    math_pain: 'Pain Index (Ulcer)',
    math_pain_expl:
      "Measures the depth and duration of drawdowns. Unlike Max Drawdown which is a single point, Pain Index captures the 'cumulative stress' of being in a亏损 state.",
    ...
    noDataSources: 'No custom sources yet',
    dataSourceName: 'Name',
  ...
  fr: {
    ...
    confirmAutoGenerate:
      'Cela générera automatiquement des combinaisons basées sur le profil seed. Continuer ?',
    copyPrefix: 'Copie- ',
    ...
    backtestWindow: 'Période de backtest',
    startMonth: 'Mois de début',
    endMonth: 'Mois de fin',
    availableDataWindow: 'Données disponibles',
    ...
    bankruptcyLtv:
      'Les stratégies suivantes ont dépassé la limite LTV maximale et ont été liquidées :',
    bankruptcyNegativeCash:
      'Les stratégies suivantes ont fait faillite en raison d\'un solde de trésorerie négatif :',
    bankruptcyWithdrawal:
      'Les stratégies suivantes ont fait faillite car le retrait dépassait la trésorerie et les actifs vendables :',
    bankruptcyExpense:
      'Les stratégies suivantes n\'ont pas pu couvrir leurs dépenses annuelles :',
    bankruptcyDesc:
      'Les stratégies suivantes ont échoué aux contrôles de solvabilité et ont été liquidées :',
    ...
    ltvChartDesc:
      'Suit le ratio Loan-to-Value (LTV) au fil du temps. Un LTV élevé augmente le risque de liquidation.',
    betaChartTitle: 'Beta du Portefeuille (Exposition au Risque)',
    betaChartDesc:
      "Beta pondéré moyen (QQQ=1, QLD=2, Cash=0) par rapport à l'actif net. Beta > 1 implique un effet de levier.",
    ...
    math_cagr: 'CAGR',
    math_cagr_expl:
      'Taux de croissance annuel composé. Mesure le taux de croissance moyen en supposant le réinvestissement des profits.',
    math_cagr_formula: 'CAGR = (Valeur Fin / Valeur Début)^(1/n) - 1',
    math_irr: 'TRI',
    math_irr_expl:
      "Taux de Rentabilité Interne. Le taux d'actualisation qui rend la VAN de tous les flux de trésorerie nulle. Résolu via Newton-Raphson.",
    ...
    math_sharpe_expl:
      'Mesure le rendement ajusté au risque. Rendement excédentaire par unité de déviation.',
    math_sharpe_formula: 'Sharpe = (Rp - Rf) / σp',
    math_calmar: 'Ratio de Calmar (basé sur le TRI)',
    math_calmar_expl:
      "Mesure le rendement par rapport au risque de retrait. L'utilisation du TRI comme numérateur prend en compte le calendrier des flux de trésorerie.",
    math_calmar_formula: 'Calmar = TRI / Drawdown Max',
    math_pain: 'Indice de Douleur (Ulcer Index)',
    math_pain_expl:
      "Mesure la profondeur et la durée des baisses. Contrairement au Drawdown Max, l'Indice de Douleur capture le 'stress cumulatif'.",
    math_pain_formula: 'Pain Index = Sqrt( Σ(Baisse_t^2) / n )',
    addAsset: 'Ajouter un actif',
    removeAsset: 'Supprimer',
    targetWeight: 'Poids cible',
    contributionWeight: 'Poids de contribution',
    pledgeRatio: 'Ratio de nantissement',
    done: 'Terminé',
    ...
    noDataSources: 'Aucune source personnalisée',
    dataSourceName: 'Nom',
    ...
    liveMonitor: 'Marché',
    backtestView: 'Backtest',
    liveMonitorTitle: 'Moniteur de liquidité QQQ',
    connectingMonitor: 'Connexion au signal sécurisé...',
    monitorDisclaimer:
      'Signaux expérimentaux d’exposition au risque et de liquidité. Pas un signal de trading en temps réel. Pas un conseil en investissement.',
  },
  'zh-CN': {
    appTitle: '投資工具',
    profiles: '配置方案',
    addProfile: '添加方案',
    autoGenerate: '自动生成组合',
    confirmAutoGenerate: '这将根据种子方案自动生成组合。是否继续？',
    copyPrefix: '复制-',
    copyProfile: '复制配置方案',
    ...
    chartColor: '图表颜色',
    strategy: '资产策略',
    backtestWindow: '回测时间窗口',
    startMonth: '开始月份',
    endMonth: '结束月份',
    availableDataWindow: '可用数据',
    initialCapital: '初始资金',
    cashYield: '现金年化收益率 (%)',
    recurringInv: '定投设置',
    amount: '金额',
    ...
    bankruptcyLtv: '以下策略因超过最大LTV限制而被清算：',
    bankruptcyNegativeCash: '以下策略因现金余额为负而破产：',
    bankruptcyWithdrawal: '以下策略因提款超过可用现金和可出售资产而破产：',
    bankruptcyExpense: '以下策略无法覆盖年度生活支出：',
    bankruptcyDesc: '以下策略未能满足检查要求并已被清算：',
    ...
    math_irr: '内部收益率 (IRR)',
    math_irr_expl:
      '考虑了资金进出时间（如定投）的真实回报率。它是使所有现金流的净现值 (NPV) 等于零的折现率，使用牛顿迭代法求解。',
    ...
    math_calmar_expl:
      '衡量收益与最大回撤风险的比值。使用内部收益率 (IRR) 作为分子，能更准确地衡量包含定投等现金流场景下的真实风险回报比。',
    math_calmar_formula: 'CalmarRatio = IRR / 最大回撤',
    math_pain: '痛苦指数 (Ulcer Index)',
    math_pain_expl:
      '衡量回撤的深度和持续时间。不同于最大回撤只记录单点最差情况，痛苦指数捕捉了处于亏损状态的“累积压力”。数值越低表示投资过程越平稳舒适。',
    math_pain_formula: 'Pain Index = √[ Σ(当日回撤%^2) / 样本总数 ]',
    addAsset: '添加资产',
    removeAsset: '移除',
    targetWeight: '目标权重',
    contributionWeight: '定投权重',
    pledgeRatio: '抵押率',
    done: '完成',
    dataRange: '数据范围',
    appDesc:
    ...
    dcaPrefix: '定投',
    dataSource: '数据源',
    addDataSource: '添加新数据源',
    noDataSources: '暂无自定义数据源',
    dataSourceName: '名称',
    dataSourceNamePlaceholder: '资产名称 (如 SPY)',
    multiplierPlaceholder: '杠杆倍数 (1, 2, 3...)',
    fileFormatHint: '格式：第一行日期 (YYYYMMDD)，第二行价格；或每行日期+价格',
    saveSource: '保存数据源',
    indexAsset: '指数资产',
    leveragedAsset: '杠杆资产',
    fileLoaded: '文件已加载',
  ...
  'zh-TW': {
    ...
    addProfile: '新增方案',
    autoGenerate: '自動生成組合',
    confirmAutoGenerate: '這將根據种子方案自動生成組合。是否繼續？',
    copyPrefix: '複製-',
    copyProfile: '複製配置方案',
    ...
    chartColor: '圖表顏色',
    strategy: '資產策略',
    backtestWindow: '回測時間窗口',
    startMonth: '開始月份',
    endMonth: '結束月份',
    availableDataWindow: '可用數據',
    initialCapital: '初始資金',
    cashYield: '現金年化收益率 (%)',
    recurringInv: '定期定額設定',
    amount: '金額',
    ...
    bankruptcyLtv: '以下策略因超過最大LTV限制而被清算：',
    bankruptcyNegativeCash: '以下策略因現金餘額為負而破產：',
    bankruptcyWithdrawal: '以下策略因提款超過可用現金和可出售資產而破產：',
    bankruptcyExpense: '以下策略無法覆蓋年度生活支出：',
    ...
    math_irr: '內部收益率 (IRR)',
    math_irr_expl:
      '考慮了資金進出時間（如定投）的真實回報率。它是使所有現金流的淨現值 (NPV) 等於零的折現率，使用牛頓迭代法求解。',
    ...
    math_calmar_expl:
      '衡量收益與最大回撤風險的比值。使用內部收益率 (IRR) 作為分子，能更準確地衡量包含定投等現金流場景下的真實風險回報比。',
    math_calmar_formula: 'CalmarRatio = IRR / 最大回撤',
    math_pain: '痛苦指數 (Ulcer Index)',
    math_pain_expl:
      '衡量回撤的深度和持續時間。不同於最大回撤只記錄單點最差情況，痛苦指數捕捉了處於虧損狀態的“累積壓力”。數值越低表示投資過程越平穩舒適。',
    math_pain_formula: 'Pain Index = √[ Σ(當日回撤%^2) / 樣本總數 ]',
    addAsset: '新增資產',
    removeAsset: '移除',
    targetWeight: '目標權重',
    contributionWeight: '定投權重',
    pledgeRatio: '抵押率',
    done: '完成',
    dataRange: '數據範圍',
    appDesc:
    ...
    dataSource: '數據源',
    addDataSource: '新增數據源',
    noDataSources: '暫無自訂數據源',
    dataSourceName: '名稱',
    dataSourceNamePlaceholder: '資產名稱 (如 SPY)',
    multiplierPlaceholder: '槓桿倍數 (1, 2, 3...)',
    fileFormatHint: '格式：第一行日期 (YYYYMMDD)，第二行價格；或每行日期+價格',
    saveSource: '儲存數據源',
    indexAsset: '指數資產',
    leveragedAsset: '槓桿資產',
    fileLoaded: '文件已加載',
